...
public interface JpClients {
    ...
    @GetMapping(value = "jp/movies", consumes = "application/json")
    List<Movie> getMovies(@RequestParam(name = "page") Integer page);

    @GetMapping(value = "jp/total", consumes = "application/json")
    int getTotal();



    /* 태그 관련 움짤 조회 */
    @GetMapping(value = "jptags/movies", consumes = "application/json")
    List<Movie> getTagMovies(@RequestParam(name = "tag") String tag, @RequestParam(name = "page") int page);

    /* 해당 태그와 관련된 개수 반환 */
    @GetMapping(value = "jptags/total", consumes = "application/json")
    ...
    void makeStory(@RequestHeader("X-AUTHORIZATION-TOKEN")String jwt,@RequestParam(name="dto") StoryDTO dto,@RequestPart(name="file") MultipartFile[] multipartFile);
