...
@FeignClient("marvelservice")
public interface MarvelClients {

    @GetMapping(value = "marvel/movie/{id}", consumes = "application/json")
    Movie getMovie(@PathVariable(name = "id") Integer id);

    @GetMapping(value = "marvel/movies", consumes = "application/json")
    List<Movie> getMovies(@RequestParam(name = "page") Integer page);

    @GetMapping(value = "marvel/total", consumes = "application/json")
    int getTotal();


    /* 태그 관련 움짤 조회 */
    @GetMapping(value = "marveltags/movies", consumes = "application/json")
    List<Movie> getTagMovies(@RequestParam(name = "tag") String tag, @RequestParam(name = "page") int page);

    /* 해당 태그와 관련된 개수 반환 */
    @GetMapping(value = "marveltags/total", consumes = "application/json")
